...
are available on CRAN (except `dualr`, which is on GitHub).
...
# Strategy 3: automatic differentiation via dualr
# (requires dualr from GitHub: remotes::install_github("queelius/dualr"))
# p3 <- mle_problem(
#   loglike = ll,
#   score   = function(theta) dualr::score(ll, theta),
#   fisher  = function(theta) dualr::observed_information(ll, theta)
# )
```
...
| Automatic differentiation (`dualr`) | Exact (machine precision) | Low | Moderate |

The general recommendation: **start with the default** (no explicit
derivatives) during model development, then **switch to `dualr`** or
hand-coded derivatives when you need accuracy or performance. For
...
| Production / accuracy-critical | `dualr` or hand-coded | Exact derivatives |
...
| [`dualr`](https://github.com/queelius/dualr) | --- | Automatic differentiation via dual numbers |
